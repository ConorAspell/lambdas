...
def get_spread(win_1=1.7, win_2=2.5, spend=1000, min_profit=0):
    spend1 = int(spend/win_1) #work out minimum needed to bet to get spend back
    spend2 = int(spend/win_2)
    solutions = []
    ...
    for i in range(spend1, spend): #goes to spend
        ...
        if return_1>=final_return and return_2>=final_return:
            solution = {}
            solution['bets'] = [i,j]
            solution['potential_total'] = return_1+return_2 #need to choose best solution, highest return seems good idea
...
def transform(odds):
    ...
    for game in every_odds:
        ...
        test = get_spread(max_team_1['team_1_wins'], max_team_2['team_2_wins'])
        if test:
            line1 = """---------------------------------------------
